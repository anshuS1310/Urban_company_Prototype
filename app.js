...
// ─── Navigation ───────────────────────────────────────────────────────────────────────────────────
// Smooth cross-fade: no blank flash, feels like a native app.
function navigateTo(name) {
  ...
  // 1. Instantly set opacity to 0 (no transition yet — instant)
  c.style.transition = 'none';
  c.style.opacity    = '0';

  // 2. After one paint frame, swap content
  requestAnimationFrame(() => {
    c.innerHTML = SCREENS[name]();
    // 3. Force a reflow so the opacity:0 is committed before we animate to 1
    void c.offsetHeight;
    // 4. Fade in smoothly — feels like a real app transition
    c.style.transition = 'opacity 0.14s ease';
    c.style.opacity    = '1';
  });
}
...
// Toast — always dark pill, white text, JS opacity fade — no browser alert()
function showToast(msg) {
  $qa('.uc-toast').forEach(t => t.remove());
  const t = document.createElement('div');
  t.className = 'uc-toast';
  // Fixed #1A1A1A background — never inverts between light/dark mode
  t.style.cssText = `
    position:absolute; bottom:68px; left:12px; right:12px;
    background:#1A1A1A; color:#FFFFFF;
    padding:11px 14px; border-radius:10px;
    font-size:13px; font-weight:600;
    z-index:300; box-shadow:0 6px 22px rgba(0,0,0,0.28);
    line-height:1.4; opacity:0;`;
  t.textContent = msg;
  $('phone-screen-container').appendChild(t);
  // JS fade-in — no pgIn keyframe needed
  void t.offsetHeight;
  t.style.transition = 'opacity 0.18s ease';
  t.style.opacity = '1';
  setTimeout(() => {
